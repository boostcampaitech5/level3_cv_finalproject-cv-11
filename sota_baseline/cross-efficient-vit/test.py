...
MODELS_DIR = "models" # opt/ml/Comb/cross/models
# BASE_DIR = "../../deep_fakes"  # opt/ml/deep_fakes
BASE_DIR = "../../sample_deep_fakes"  # opt/ml/deep_fakes
DATA_DIR = os.path.join(BASE_DIR, "dataset") # opt/ml/deep_fakes/dataset
# TEST_DIR = os.path.join(DATA_DIR, "test_set") # opt/ml/deep_fakes/dataset/test_set
TEST_DIR = os.path.join(DATA_DIR, "sample_test_set") # opt/ml/deep_fakes/dataset/test_set
# OUTPUT_DIR = os.path.join(MODELS_DIR, "tests") # opt/ml/Comb/cross/models/tests
OUTPUT_DIR = os.path.join(MODELS_DIR, "sample_tests") # opt/ml/Comb/cross/models/tests

# TEST_LABELS_PATH = os.path.join(BASE_DIR, "dataset/dfdc_test_labels.csv") # opt/ml/deep_fakes/dataset/dfdc_kaggle_labels.csv
TEST_LABELS_PATH = "/opt/ml/Combining-EfficientNet-and-Vision-Transformers-for-Video-Deepfake-Detection/cross-efficient-vit/dfdc_kaggle_labels.csv" 
...
def read_frames(video_path, videos): # video_path = opt/ml/deep_fakes/dataset/test_set/DFDC/각 비디오 폴더 
    # videos = 그냥 빈 리스트
    
    # Get the video label based on dataset selected
    # 이거 할 필요 없음
    # method = get_method(video_path, DATA_DIR) # opt/ml/deep_fakes/dataset/manipulated_sequence
    # print('==========method===========',method)
    method = "DFDC"
    if "Original" in video_path: # 이거 필요 없을 것 같다.
        label = 0.
    elif method == "DFDC":
        test_df = pd.DataFrame(pd.read_csv(TEST_LABELS_PATH))
        video_folder_name = os.path.basename(video_path) # 각 비디오 폴더 이름
...
if __name__ == "__main__":
    ...
    # 모델 경로 수정
    parser.add_argument('--model_path', default='/opt/ml/Combining-EfficientNet-and-Vision-Transformers-for-Video-Deepfake-Detection/cross-efficient-vit/cross_efficient_vit.pth', type=str, metavar='PATH',
                        help='Path to model checkpoint (default: none).')
    parser.add_argument('--dataset', type=str, default='DFDC', # 내가 수정
    ...
    # folders = ['/opt/ml/Combining-EfficientNet-and-Vision-Transformers-for-Video-Deepfake-Detection/samples'] # 내가 수정
    for folder in folders: # ["DFDC"]
        method_folder = os.path.join(TEST_DIR, folder)  # opt/ml/deep_fakes/dataset/test_set/DFDC 
        # method_folder = folder # 내가 수정
        for index, video_folder in enumerate(os.listdir(method_folder)): # opt/ml/deep_fakes/dataset/test_set/DFDC 이 안에 있는 비디오 폴더들
            paths.append(os.path.join(method_folder, video_folder)) # opt/ml/deep_fakes/dataset/test_set/DFDC/각 비디오 폴더 
    # print(paths)
    # Read faces
    with Pool(processes=cpu_count()-1) as p:
        with tqdm(total=len(paths)) as pbar:
            for v in p.imap_unordered(partial(read_frames, videos=videos),paths): # paths = opt/ml/deep_fakes/dataset/test_set/DFDC/각 비디오 폴더 
                pbar.update()
    # print(videos.shape)
    # print(len(videos[0]))
    # print(videos[0][0])
    ...
    # print(videos[0][2])
    # exit(0)
    video_names = np.asarray([row[2] for row in videos]) # ../../sample_deep_fakes/dataset/sample_test_set/DFDC/aagfhgtpmv
    correct_test_labels = np.asarray([row[1] for row in videos]) # 정답 1 가짜인지, 0 진짜인지
    videos = np.asarray([row[0] for row in videos]) # 비디오 
    preds = []
    # print(videos.shape)
    # exit(0)
    ...
    f = open("sample" + opt.dataset + "_" + model_name + "_labels.txt", "w+")
