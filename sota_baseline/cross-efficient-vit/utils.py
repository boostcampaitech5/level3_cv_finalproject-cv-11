...
def get_method(video, data_path): # video_path(video) = opt/ml/deep_fakes/dataset/test_set/DFDC/각 비디오 폴더 
                                          # data_path = opt/ml/deep_fakes/dataset
    methods = os.listdir(os.path.join(data_path, "manipulated_sequences")) # opt/ml/deep_fakes/dataset/manipulated_sequences 이 안에 있는 파일을 다 리스트
    methods.extend(os.listdir(os.path.join(data_path, "original_sequences"))) # opt/ml/deep_fakes/dataset/original_sequences 이 안에 있는 파일을 다 리스트에 추가
    # 내가 추가
