...
# 생성하기 이미지 업로드
@file_router.post("/generation-source-upload") # 생성하기-대상이미지업로드버튼
async def upload_files(files: List[UploadFile] = File(...), user_info): # user_info - dict으로된 정보
    # 파일 수 확인 
    if len(files) > 1:
...
@file_router.post("/generation-target-upload") # 생성하기-대상이미지업로드버튼
async def upload_files(files: List[UploadFile] = File(...), user_info): # user_info - dict으로된 정보
    # 파일 수 확인 
    if len(files) > 1:
        return ('배경 이미지는 한 장만 넣어주세요')
...
# 탐지하기 이미지 업로드
@file_router.post("/generation-upload") # 생성하기-이미지업로드버튼
async def upload_files(files: List[UploadFile] = File(...), user_info): # user_info - dict으로된 정보
    # 유저 폴더 생성
    user_directory = f'../data/generation/{user_info['username']}_{user_info['user_id']}' # ex) johndoe_1
    make_folder(user_directory)
    ...
    return {"filenames": [file.filename for file in files], "message": f"{len(files)} Files uploaded successfully."}
...
## html 
@file_router.get("/generation-upload")
def main():
    content = """
<form action="/generation-upload" enctype="multipart/form-data" method="post">
<input name="files" type="file" multiple>
<input type="submit">
</form>
</body>
